...
const PurchaseReturnTab = () => {
  ...
  // -------------------- Compute totals --------------------
  useEffect(() => {
    const sub = returnItems.reduce((acc, item) => acc + (item.total || 0), 0);
    setSubtotal(sub);
  ...
  const removeRow = key => setReturnItems(returnItems.filter(row => row.key !== key));
  ...
  const handleSelectItem = (key, itemId) => {
    ...
    if (returnItems[returnItems.length - 1].key === key) addRow();
  };

  const handleSaveDraft = () => {
    if (!selectedVendor) return message.error("Select a vendor first");
    if (!returnItems.some(i => i.itemId)) return message.error("Add at least one item");
  ...
  const handleSubmit = async values => {
    try {
      ...
      message.error(err.response?.data?.message || "Failed to create purchase return");
    }
  };
      ...
      <div style={{ display: "flex", gap: 20, padding: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <Card title="Return Summary">
            ...
            <p>Discount Amount: ₹{returnItems.reduce((acc, i) => acc + (i.discount || 0), 0).toFixed(2)}</p>
            ...
            <Button type="primary" onClick={handleSaveDraft} style={{ marginRight: 10 }}>Create Return</Button>
