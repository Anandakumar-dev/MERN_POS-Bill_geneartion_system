// -------------------------------------------
// SalesReturnChallanTab.js
// POS-style Sales Return Challan Page
...
const SalesReturnChallanTab = () => {
  ...
  const TYPE = "Sales Return Challan"; // preset type
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        setItemsList(data);
        if (returnItems.length === 0) addRow();
      } catch (err) {
        console.error(err);
  ...
  // -------------------- Compute totals --------------------
  useEffect(() => {
    let sub = returnItems.reduce((acc, item) => acc + (item.total || 0), 0);
    setSubtotal(sub);
    const total = sub + Number(shippingCharges || 0);
    setFinalTotal(total);
  }, [returnItems, shippingCharges, discountPercent]);
  ...
  const removeRow = (key) => {
    setReturnItems(returnItems.filter((row) => row.key !== key));
  };

  // -------------------- Row changes --------------------
  const handleRowChange = (key, field, value) => {
    const updated = returnItems.map((row) => {
      if (row.key === key) {
  ...
  const handleSelectItem = (key, itemId) => {
    ...
    const isLastRow = returnItems[returnItems.length - 1]?.key === key;
    if (isLastRow) addRow();
  };

  // -------------------- Save Draft / Open Customer Modal --------------------
  const handleSaveDraft = () => {
    if (!returnItems.some((i) => i.itemId))
  ...
  const handleSubmit = async (values) => {
    try {
      const validItems = returnItems.filter((i) => i.itemId);
      if (!validItems.length)
        return message.error("Add at least one item before processing the return challan");
      ...
      message.error(err.response?.data?.message || `Failed to process ${TYPE}`);
    }
  };
  ...
  const columns = [
    ...
    { title: "Discount", dataIndex: "discount", render: (text, record) => (
        <InputNumber min={0} value={record.discount} onChange={(val) => handleRowChange(record.key, "discount", val)} />
      ),
    },
    { title: "Total", dataIndex: "total", render: (text, record) => `₹${record.total?.toFixed(2) || 0}` },
  ...
  return (
    <DefaultLayout>
      <div style={{ display: "flex", gap: 20, padding: 20 }}>
        <div style={{ flex: 3 }}>
          <Table dataSource={returnItems} columns={columns} pagination={false} rowKey="key" />
        </div>
        ...
        <div style={{ flex: 1 }}>
          <Card title="Return Challan Summary">
            ...
            <p>
              Discount %: <InputNumber min={0} max={100} value={discountPercent} onChange={setDiscountPercent} />
            </p>
            <p>
              Discount Amount: ₹{returnItems.reduce((acc, i) => acc + (i.discount || 0), 0).toFixed(2)}
            </p>
            <p>
            ...
            <Button type="primary" onClick={handleSaveDraft} style={{ marginRight: 10 }}>
              Process Return Challan
            </Button>
            <Button danger onClick={() => setReturnItems([])}>Cancel</Button>
          </Card>
        </div>
      </div>
      ...
      <Modal title="Customer Details" open={customerModal} onCancel={() => setCustomerModal(false)} footer={false}>
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <Button type="primary" htmlType="submit">Confirm Return Challan</Button>
          </div>
        </Form>
      </Modal>
    </DefaultLayout>
...
export default SalesReturnChallanTab;
